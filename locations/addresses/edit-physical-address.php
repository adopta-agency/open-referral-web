...
$clean = filter_input_array(INPUT_GET,$_GET); 		
$location_id = $clean['location_id'];
$physical_address_id = $clean['physical_address_id'];

require_once "../nav.php";

$api_url = $api_base_url . "locations/" . $location_id . "/physical_address/" . $physical_address_id . "/";
//echo $api_url;
// Update PhysicalAddress
if(isset($clean['edit-physical_address']))
	{
	...
	<p class="bg-success" style="text-align: center; font-weight: bold; padding: 5px;">
		The physical address has been saved!
	</p>
	<?php
	curl_close($http);			
	}		
	...
	$physical_address = json_decode($response,true);	
	$attention = $physical_address['attention'];
	$address_1 = $physical_address['address_1'];
	$address_2 = $physical_address['address_2'];
	$address_3 = $physical_address['address_3'];
	$address_4 = $physical_address['address_4'];
...
?>
<form method="get" action="edit-physical-address.php" name="SavePhysicalAddressForm">
  <input type="hidden" class="form-control" id="location_id" name="location_id" value="<?php echo $location_id; ?>">
  <input type="hidden" class="form-control" id="physical_address_id" name="physical_address_id" value="<?php echo $physical_address_id; ?>">
  ...
  <button onclick="document.SavePhysicalAddressForm.submit();" type="submit" class="btn btn-default" name="edit-physical_address" value="1">Save Any Changes</button>
</form>
